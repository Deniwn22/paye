...
type PaystackService struct {
	repo            *PaystackRepository
	providerRepo    *providers.ProviderRepo
	encryptionKey   string
	paystackBaseURL string
}

func NewPaystackService(repo *PaystackRepository, providerRepo *providers.ProviderRepo, encryptionKey string) *PaystackService {
...
func (s *PaystackService) SetPaystackBaseURL(url string) {
...
func (s *PaystackService) getPaystackClient(ctx context.Context, projectID string) (*paystack.Paystack, error) {
	...
	client := paystack.New(decryptedSecret)
	if s.paystackBaseURL != "" {
		client.BaseURL = s.paystackBaseURL
	}
	return client, nil
}

// Refund performs a transaction refund and persists the refund record
func (s *PaystackService) Refund(ctx context.Context, projectID string, req providers.RefundRequest) (*providers.RefundResponse, error) {
	...
	client, err := s.getPaystackClient(ctx, projectID)
	if err != nil {
		return nil, err
	}
...
// CreateTransferRecipient creates a transfer recipient on Paystack and persists the recipient details
func (s *PaystackService) CreateTransferRecipient(ctx context.Context, projectID string, req providers.TransferRecipientRequest) (*providers.TransferRecipientResponse, error) {
	client, err := s.getPaystackClient(ctx, projectID)
	if err != nil {
		return nil, err
	}
...
// InitiateTransfer initiates a balance transfer on Paystack and persists the transfer record
func (s *PaystackService) InitiateTransfer(ctx context.Context, projectID string, req providers.TransferRequest) (*providers.TransferResponse, error) {
	client, err := s.getPaystackClient(ctx, projectID)
	if err != nil {
		return nil, err
	}
...
// CreatePlan creates a subscription plan on Paystack and persists the plan details
func (s *PaystackService) CreatePlan(ctx context.Context, projectID string, req providers.PlanRequest) (*providers.PlanResponse, error) {
	client, err := s.getPaystackClient(ctx, projectID)
	if err != nil {
		return nil, err
	}
...
// CreateSubscription creates a subscription on Paystack and persists the subscription record
func (s *PaystackService) CreateSubscription(ctx context.Context, projectID string, req providers.SubscriptionRequest) (*providers.SubscriptionResponse, error) {
...
// CancelSubscription cancels an active subscription on Paystack and updates the local record status
func (s *PaystackService) CancelSubscription(ctx context.Context, projectID string, subscriptionCode string, emailToken string) error {
