...
import WebhookLogsTable, { WebhookLog } from "@/components/webhook-logs-table"
...
export default function WebhooksManager({
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    // Note: targetUrl is optional for local-only logging
  ...
  return (
    <div className="space-y-6 text-sm font-sans select-text">
      ...
      {activeTab === "routes" ? (
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              ...
              <DialogContent className="max-w-md bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-2xl p-6">
                ...
                <form onSubmit={handleAddSubmit} className="space-y-4 pt-3">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider block">Where to send notifications (Optional - Leave blank for local logs only)</label>
                    <input
                      type="url"
                      value={targetUrl}
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="px-3.5 py-2.5 bg-zinc-50/50 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-900 font-mono text-xs text-zinc-700 dark:text-zinc-300 truncate select-all rounded-lg max-w-3xl">
                            {c.target_url || "Stored locally only (No forwarding URL configured)"}
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div className="space-y-6 animate-in fade-in duration-200">
          <div>
            <h2 className="text-sm font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">Webhook Logs History</h2>
            <p className="text-xs text-zinc-500 dark:text-zinc-500 mt-1 font-medium">Inspect raw payload details for notifications captured by Paye.</p>
          </div>
          <WebhookLogsTable logs={initialLogs} />
        </div>
      )}
    </div>
  )
